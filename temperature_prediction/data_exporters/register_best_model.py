...
def train_and_register(
    ...
    **kwargs,
) -> Tuple[Booster, float, Series, str]:
    """
    ...
    runs = track_experiment(
        block_uuid='register_best_model',
    ...
    """
    update_model_version(
        client, model_details.name, model_details.version, version_description
    )
    # Transition the model to Staging
    transition_to_stage(
        client, model_details.name, model_details.version, "Staging", False
    )
    # set_global_variable('training', 'staging_model_version', model_details.version)

    return model, metrics['rmse'], y_pred, model_details.version
